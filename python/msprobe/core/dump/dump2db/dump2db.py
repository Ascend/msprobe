...
def scan_files(data_dir):
    ...
    for step_dir in os.listdir(data_dir):
        ...
        for item_dir in os.listdir(step_path):
            if item_dir.startswith('rank') or item_dir.startswith('proc'):
                ...
                if not os.path.isdir(item_path):
                    skipped_item_dirs.append(item_dir)
                    continue
                try:
                    ...
                    skipped_item_dirs.append(item_dir)
                    continue
                valid_item_dir_found = True
                json_path = os.path.join(item_path, "dump.json")
                if not os.path.isfile(json_path):
                    logger.warning(f"No 'dump.json' file found in: {item_path}")
                    continue
        ...
        if not valid_item_dir_found:
            skipped_repr = (
                ", ".join(skipped_item_dirs) if skipped_item_dirs else "(none — no rank*/proc* entries at all)"
            )
            logger.warning(
                f"No valid 'rank*' or 'proc*' directories found in: {step_path}. "
                f"Expected directory names like 'rank0' or 'proc0'. Skipped entries: {skipped_repr}"
            )
