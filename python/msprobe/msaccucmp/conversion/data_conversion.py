...
class DumpDataConversion:
    ...
    def __init__(self: any) -> None:
        ...
        parse.add_argument(
            "-target", dest="target", help="<Required> the target after conversion, numpy or dump", required=True
        )
        parse.add_argument(
            "-type", dest="type", help="<Required> the type for data, caffe, tf, quant or offline", required=True
        )
        parse.add_argument(
            "-i", dest="input_path", type=safe_path_string, help="<Required> the input path", required=True
        )
        parse.add_argument(
            "-o",
            dest="output_path",
    ...
    def _get_op_name_from_path(self: any, file_path: str) -> str:
        ...
        layer_name = (
            self.layer_name_switch[self.type](layer_name) if self.type in self.layer_name_switch else layer_name
        )
        return layer_name
    ...
    def _save_tensor_to_file(self: any, tensor_list: list, name: str, input_path: str, tensor_type: str) -> None:
        ...
        for index, tensor in enumerate(tensor_list):
            log.print_info_log('Start to convert %s:%d of "%r" to numpy file.' % (tensor_type, index, input_path))
            array = tensor.data
            try:
            ...
            if self.type in ("caffe", "tf", "quant"):
            ...
            log.print_info_log(
                'The %s:%d of "%r" has been converted to file "%r".'
    ...
    def _save_buffer_to_file(self: any, tensor_list: list, name: str, input_path: str) -> None:
        ...
        for index, tensor in enumerate(tensor_list):
            ...
            log.print_info_log('Start to convert %sbuffer:%d of "%r" to bin file.' % (buffer_type, index, input_path))
            file_name = "%s.%sbuffer.%s.bin" % (name, buffer_type, index)
            output_dump_path = os.path.join(self.output_path, file_name)
            try:
                with os.fdopen(
                    os.open(output_dump_path, ConstManager.WRITE_FLAGS, ConstManager.WRITE_MODES), 'wb'
                ) as output_file:
                    output_file.write(tensor.data)
                    log.print_info_log(
                        'The %sbuffer:%d of "%r" has been converted to file "%r".'
        ...
        for index, tensor in enumerate(tensor_list):
            ...
            try:
                with os.fdopen(
                    ...
                    output_file.write(tensor.data)
                    log.print_info_log(
                        'The space:%d of "%r" has been converted to file "%r".' % (index, input_path, output_dump_path)
                    )
            except IOError as io_error:
                log.print_error_log('Failed to open "%r". %s ' % (output_dump_path, str(io_error)))
            finally:
    ...
    def _convert_file_exec(self: any, input_path: str) -> None:
        ...
        if self.target == "dump":
            ...
            log.print_info_log('The file "%r" has been converted to file "%r".' % (input_path, output_dump_path))
