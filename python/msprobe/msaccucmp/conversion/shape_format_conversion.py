...
class ShapeConversionMain:
    ...
    def __init__(self: any) -> None:
        ...
        parse.add_argument(
            "-format", dest="format", default="", help="<Required> the format to transfer", required=True
        )
        parse.add_argument(
            "-o",
            dest="output_path",
        ...
        parse.add_argument('-tensor', dest="tensor", default="output", help="<Optional> the tensor, input or output")
        parse.add_argument('-index', dest="index", default="0", help="<Optional> the index for tensor")
        parse.add_argument(
            '-shape',
            dest="shape",
        ...
        if format_str not in ConstManager.STRING_TO_FORMAT_MAP:
            log.print_error_log('The format "%r" is not supported. Please check the format.' % format_str)
            raise CompareError(CompareError.MSACCUCMP_INVALID_PARAM_ERROR)
    ...
    def check_arguments_valid(self: any) -> int:
        ...
        ret = path_check.check_path_valid(self.dump_file_path, True, False, path_check.PathType.File)
        if ret != CompareError.MSACCUCMP_NONE_ERROR:
            return ret
        ...
        log.print_info_log(
            'The target format is %s for %r:%d.' % (common.get_format_string(self.format_to), self.tensor, self.index)
        )
    ...
    def _convert_format(self: any, tensor: any, index: int) -> None:
        if tensor.tensor_format == self.format_to:
            log.print_info_log(
                "There is no need to transfer format because the format (%s) is the same."
        ...
        if tensor.tensor_format in (DD.FORMAT_HWCN, DD.FORMAT_NCHW, DD.FORMAT_NHWC):
        ...
        log.print_info_log(
            "Before transferring the format, the dump data is %s%s."
            % (common.get_format_string(real_format), utils.convert_shape_to_string(tensor.shape))
        )
        ...
        dump_data_np = self.manager.execute_format_convert(
            ...
            dump_data_array,
            {'group': common.get_sub_format(tensor)},
        )
        log.print_info_log(
            "After transferring the format, the dump data is %s%s."
            % (common.get_format_string(self.format_to), utils.convert_shape_to_string(dump_data_np.shape))
        )
        ...
        output_file_path = os.path.join(
            ...
            '%s.%s.%d.%s.npy'
            % (
                os.path.basename(self.dump_file_path),
                self.tensor,
                index,
        ...
        log.print_info_log('The dump data for %r:%d has been saved to "%r".' % (self.tensor, index, output_file_path))
...
class FormatConversionMain:
    ...
    def __init__(self: any, arguments: any = None) -> None:
        ...
        self.attr = {'dump_version': arguments.dump_version, 'output_file_type': arguments.output_file_type}
    ...
    def check_arguments_valid(self: any) -> int:
        ...
        self.multi_process = MultiConvertProcess(self._convert_format_for_one_file, self.input_path, self.output_path)
        ...
        log.print_info_log('The target format is "%s".' % common.get_format_string(self.format_to))
    ...
    def _save_to_file(self: any, *args: any) -> None:
        ...
        if self.attr.get('output_file_type') == 'npy':
            file_name = '%s.%s.%d.%s.npy' % (
                os.path.basename(dump_file_path),
                tensor_type,
                index,
                utils.get_string_from_list(dump_data_np.shape, 'x'),
            )
        ...
        FileUtils.save_array_to_file(
            output_file_path, dump_data_np, self.attr.get('output_file_type') != 'bin', dump_data_np.shape
        )
        log.print_info_log('The data of %s:%s has been saved to "%r".' % (tensor_type, index, output_file_path))

    def _get_format_and_shape(self: any, tensor: any, index: int, tensor_type: str, dump_file_path: str):
        # check the 4-dimension valid
        real_format = tensor.tensor_format
        if tensor.tensor_format in (DD.FORMAT_HWCN, DD.FORMAT_NCHW, DD.FORMAT_NHWC):
            if len(tensor.shape) != ConstManager.FOUR_DIMS_LENGTH:
    ...
    def _convert_format_for_one_tensor(self: any, *args: any) -> None:
        ...
        log.print_info_log(
            ...
            % (tensor_type, index, common.get_format_string(real_format), utils.convert_shape_to_string(tensor.shape))
        )
        ...
        if real_format == self.format_to:
            log.print_info_log(
                ...
                % (common.get_format_string(tensor.tensor_format), tensor_type, index, dump_file_path)
            )
        ...
        try:
            if len(shape_to) > 0 and real_format != self.format_to:
                ...
                log.print_info_log(
                    'The data of %s:%d has been sliced from %s to %s.'
                    % (tensor_type, index, old_shape_str, utils.convert_shape_to_string(shape_to))
                )
        except (OSError, SystemError, ValueError, TypeError, RuntimeError, MemoryError, CompareError):
            log.print_error_log(
                'Failed to slice data for %r from %s to %s.'
                % (
                    dump_file_path,
                    utils.convert_shape_to_string(dump_data_np.shape),
    ...
    def _save_file_for_convert_failed_tensor(self: any, *args: any) -> None:
        ...
        if self.attr.get('output_file_type') == 'msnpy':
            ...
            output_file_path = os.path.join(self.output_path, file_name)
            FileUtils.save_array_to_file(
                output_file_path, tensor.data, self.attr.get('output_file_type') != 'bin', tensor.shape
            )
            log.print_info_log('The data of %s:%d has been parsed into "%r".' % (tensor_type, index, output_file_path))

    def _convert_format_for_tensor(
        self: any, tensor_list: list, dump_file_path: str, tensor_type: str, op_name: str
    ) -> (int, str):
        ...
        for index, tensor in enumerate(tensor_list):
            try:
                ...
                log.print_error_log(
                    'Failed to convert format for %s:%d of "%r". %s' % (tensor_type, index, dump_file_path, error)
                )
                ...
                log.print_error_log('Failed to convert format for %s:%d of "%r"' % (tensor_type, index, dump_file_path))
                ret = error.code
                msg += ',%s:%d' % (tensor_type, index)
                self._save_file_for_convert_failed_tensor(tensor, dump_file_path, tensor_type, index, op_name)
            finally:
    ...
    def _convert_format_for_input_or_output(self: any, dump_data: DumpDataObj, dump_file_path: str) -> None:
        ...
        if self.one_file_info.get('index') >= len(tensor_list):
            log.print_error_log(
                ...
                % (self.one_file_info.get('tensor'), self.one_file_info.get('index'), len(tensor_list))
            )
            raise CompareError(CompareError.MSACCUCMP_INDEX_OUT_OF_BOUNDS_ERROR)
        self._convert_format_for_one_tensor(
            tensor_list[self.one_file_info.get('index')],
            self.one_file_info.get('index'),
            self.one_file_info.get('tensor'),
            dump_file_path,
            dump_data.op_name,
        )
    ...
    def _convert_format_for_tensor_list(self: any, dump_data: DumpDataObj, dump_file_path: str) -> (int, str):
        ret = CompareError.MSACCUCMP_NONE_ERROR
        msg = dump_file_path
        ret_input, msg_input = self._convert_format_for_tensor(
            dump_data.input_data, dump_file_path, ConstManager.INPUT, dump_data.op_name
        )
        if ret_input != CompareError.MSACCUCMP_NONE_ERROR:
            ret = ret_input
        msg += msg_input
        ret_output, msg_output = self._convert_format_for_tensor(
            dump_data.output_data, dump_file_path, ConstManager.OUTPUT, dump_data.op_name
        )
