...
class FileUtils:
    ...
    def save_file(file_path: str, content: any) -> None:
        ...
        if os.path.islink(file_path):
            os.unlink(file_path)
        try:
            with os.fdopen(os.open(file_path, ConstManager.WRITE_FLAGS, ConstManager.WRITE_MODES), 'w+') as output_file:
    ...
    def load_json_file(cls: any, json_file: str) -> any:
        ...
        try:
            ...
            log.print_error_log('Failed to load json object. The content of the json file "%r" is invalid.' % json_file)
...
class OverflowFileUtils(FileUtils):
    ...
    DEBUG_FILE_PATTERN = (
        r"(Opdebug)\.(Node_OpDebug)\.([0-9]+)\.?([0-9]+)?\.([0-9]{1,255})"
        r"\.?([0-9]+)?\.?([0-9]+)?\.?([0-9]+)?\.?([0-9]+)?"
    )
    PARSED_DEBUG_FILE_PATTERN = (
        r"Opdebug\.Node_OpDebug\.([0-9]+)\.?([0-9]+)?"
    ...
    @staticmethod
    def _list_file_with_pattern(path: str, pattern: str, extern_pattern: any, gen_info_func: any) -> dict:
        """
    ...
    def _gen_dump_file_info(name: str, dir_path: str, match: any) -> any:
        ...
        if len(match.groups()) > 4:
            file_desc = {"file_path": os.path.join(dir_path, name), "timestamp": int(match.groups()[TIMESTAMP])}
            dump_attr = {
    ...
    @staticmethod
    def _gen_one_tensor_desc(output_path: str, dump_file_desc: any, tensor_list: any, tensor_type: str = '') -> dict:
        """
        ...
        for index, tensor in enumerate(tensor_list):
            parsed_dump_file_name = "{}.{}.{}.npy".format(dump_file_name, tensor_type, index)
            file_desc = {
            ...
            parsed_dump_files[parsed_dump_file_name] = ParsedDumpFileDesc(file_desc, dump_attr, anchor)
    ...
    def _gen_parsed_debug_file_info(name: str, dir_path: str, match: any) -> object:
        ...
        file_desc = {"file_path": os.path.join(dir_path, name), "timestamp": int(match.groups()[2])}
        dump_attr = {
        ...
        anchor = {"anchor_type": match.groups()[-2], "anchor_idx": int(match.groups()[-1])}
    ...
    def parse_mapping_csv(self: any, path: str, pattern: str, extern_pattern: any = '') -> dict:
        ...
        for item in mapping:
            ...
            if not os.path.isfile(src_file):
                log.print_warn_log("the file %r in mapping.csv is not exist, dir: %r." % (item[hash_index], path))
                continue
            match = re_pattern.match(item[file_name_index])
            if match is None:
                continue
            if extern_pattern != '' and not re.match(extern_pattern, item[file_name_index]):
                continue

            matched_files[item[hash_index]] = self._gen_dump_file_info(item[hash_index], path, match)
        return matched_files
    ...
    def list_dump_files(self: any, dump_path: str, pattern: str, extern_pattern: any = '') -> any:
        ...
        :extern_pattern: the extern_pattern
        """
        npu_dump_files = self._list_file_with_pattern(dump_path, pattern, extern_pattern, self._gen_dump_file_info)
        npu_dump_files.update(self.parse_mapping_csv(dump_path, pattern, extern_pattern))
    ...
    def list_parsed_dump_files(self: any, output_path: str, dump_file_desc: any) -> dict:
        ...
        parsed_dump_files = self._gen_one_tensor_desc(output_path, dump_file_desc, dump_data.input_data, 'input')
        parsed_dump_files.update(
            self._gen_one_tensor_desc(output_path, dump_file_desc, dump_data.output_data, 'output')
        )
        parsed_dump_files.update(self._gen_one_tensor_desc(output_path, dump_file_desc, dump_data.buffer))
        return parsed_dump_files
    ...
    def list_parsed_debug_files(self: any, dir_path: str, extern_pattern: str = '') -> dict:
        ...
        return self._list_file_with_pattern(
            dir_path, self.PARSED_DEBUG_FILE_PATTERN, extern_pattern, self._gen_parsed_debug_file_info
        )
...
class ParsedDumpFileDesc(DumpFileDesc):
    ...
    def __init__(self: any, file_desc: dict, dump_attr: dict, anchor: dict) -> None:
        super().__init__(file_desc, dump_attr)
