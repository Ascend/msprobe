...
class TLV:
    ...
    def _check_tl(tag, length, tlv_cfg):
        ...
        if tag_in_cfg != tag:
            log.print_error_log("Failed to parse of tag_in_cfg: %s, tag_in_stream: %s." % (tag_in_cfg, tag))
            raise CompareError(CompareError.MSACCUCMP_PARSE_NANO_DUMP_FILE_ERROR)
    ...
    def parse_tlv_by_cfg_tab(self, content: bytes, x: object) -> (bytes, object):
        ...
        for tlv_cfg in self.tlv_tab:
            ...
            if tlv_type == "ATOM":
                ...
                if tlv_cfg.get("Name") == "tlv_list_len":
                    tlv_remaining = getattr(x, "tlv_list_len", -1)
            elif tlv_type == "TLV":
                aux, x = self._unpack_tlv_value(aux, tlv_cfg, x)
            elif tlv_type == "TLNV":
                if tlv_remaining != -1 and tlv_remaining <= 0:
                    setattr(x, tlv_cfg.get("Name"), None)
                else:
                    aux_before = len(aux)
                    aux, x = self._unpack_tlnv_value(aux, tlv_cfg, x)
                    if tlv_remaining >= 0:
                        tlv_remaining -= aux_before - len(aux)
            elif tlv_type == "NV":
    ...
    def _unpack_atom_value(self, aux: bytes, tlv_cfg, x: object) -> (bytes, object):
        ...
        if isinstance(ele_type, list):
            ...
            try:
                ...
                log.print_error_log(
                    "Failed to parse of ele name: %s, value_type: %s, "
    ...
    def _unpack_nv_value(self, aux: bytes, tlv_cfg, x: object, ele_num) -> (bytes, object):
        ele_type, _ = tlv_cfg.get("Ele_Type"), tlv_cfg.get("Name")
        value = []
