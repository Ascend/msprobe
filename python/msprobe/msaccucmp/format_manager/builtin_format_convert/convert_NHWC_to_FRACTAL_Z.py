...
def convert(shape_from: list, shape_to: list, array: any, group: int = 1) -> any:
    ...
    for g_axis, n_axis, h_axis, w_axis, c_axis in product(
        range(group), range(n_ori), range(kh_axis), range(kw_axis), range(c_ori)
    ):
        ...
        array_to[
            _get_axis(
                [g_axis, n_axis, h_axis], {'e_multi': e_multi, 'kh_axis': kh_axis, 'kw_axis': kw_axis}, dst_c, w_axis
            )
        ][dst_n // ConstManager.N0_AXIS][dst_n % ConstManager.N0_AXIS][dst_c % ConstManager.C0_AXIS] = array_shape[
            src_n
        ][h_axis][w_axis][c_axis]
    return array_to
