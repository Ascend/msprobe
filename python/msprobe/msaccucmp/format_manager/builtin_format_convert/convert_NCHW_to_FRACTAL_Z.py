...
def convert(shape_from: list, shape_to: list, array: any, group: int = 1) -> any:
    ...
    for g_axis, n_axis, c_axis, h_axis, w_axis in product(
        range(group), range(n_ori), range(c_ori), range(kh_axis), range(kw_axis)
    ):
