...
from vector_cmp.compare_detail.detail_writer import DetailWriter  # pylint: disable=ungrouped-imports
...
from conversion.tensor_conversion import TensorConversion  # pylint: disable=ungrouped-imports
...
class DetailComparison:
    ...
    def __init__(
        self: any, detail_info: DetailInfo, fusion_op_comparison: FusionOpComparison, output_path: str
    ) -> None:
    ...
    def check_index_valid(self: any, my_output_data: any) -> None:
        ...
        if self.detail_info.tensor_id.index >= count:
            log.print_out_of_range_error(
                ...
                self.detail_info.tensor_id.tensor_type,
                self.detail_info.tensor_id.index,
                '[0, %d)' % count,
            )
    ...
    def compare(self: any) -> int:
        ...
        log.print_info_log('[%s] Start to compare detail for %s.' % (self.detail_info.tensor_id.op_name, tensor_id))
        ...
        try:
            ground_truth_tensor = self.fusion_op_comparison.get_right_dump_data(
        ...
        self.detail_info.set_detail_format(
            utils.convert_shape_to_string(tensor_list[self.detail_info.tensor_id.index].shape),
        ...
        tensor_conversion = TensorConversion(self.fusion_op, self.fusion_op_comparison.format_manager, is_detail=True)
        my_output_array, ground_truth_array, my_output_shape = tensor_conversion.get_my_output_and_ground_truth_data(
            self.fusion_op_comparison.compare_data, tensor_list[self.detail_info.tensor_id.index], ground_truth_tensor
        )
    ...
    def _get_my_output_tensor_by_op(self: any, relation: int) -> any:
        my_output_data_path, my_output_data = self.fusion_op_comparison.compare_data.get_left_dump_data(
            self.fusion_op.op_name
        )
    ...
    def _get_my_output_tensor_list(self: any) -> list:
        ...
        if self.fusion_op.attr.quant_filter:
            log.print_error_log(
                "[{}] The -op param should not specify an operator in quant/dequant pair.".format(
                    self.fusion_op.op_name
                )
            )
            raise CompareError(CompareError.MSACCUCMP_UNSUPPORTED_COMPARE_ERROR)
...
class DumpDetailComparison:
    ...
    def __init__(self: any, detail_info: DetailInfo, compare_data: dump.CompareData, output_path: str) -> None:
        self.detail_info = detail_info
    ...
    def compare(self: any) -> int:
        ...
        log.print_info_log('[%s] Start to compare detail for %s.' % (op_name, tensor_id))
        ...
        if my_output_shape != ground_truth_shape:
            log.print_error_log(
                "My Output data shape %s not equal to Ground Truth data shape %s."
        ...
        self.detail_info.check_and_set_format(
            utils.convert_shape_to_string(left_tensor_data[tensor_index].shape),
