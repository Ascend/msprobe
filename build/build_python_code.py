...
def prepare_third_party_tool():
    ...
    cmd = [prepare_shell]
    logging.info("--------------------start compiling protobuf--------------------")
    prepare_protoc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def generate_dump_data_api():
    ...
    cmd = [protoc_dir, "-I=" + dump_proto_dir, "--python_out=" + src_compare_path, dump_proto_path]
    gen_api = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    if gen_api.returncode != 0:
        return gen_api.returncode
...
def main():
    ...
    returncode = prepare_third_party_tool()
    if returncode != 0:
        return returncode
    ...
    returncode = generate_dump_data_api()
    if returncode != 0:
        return returncode
