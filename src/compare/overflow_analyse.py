...
class OverflowAnalyse:
    ...
    def check_argument(arguments: any = None) -> int:
        ...
        if ret != CompareError.MSACCUCMP_NONE_ERROR:
            ...
            ret = utils.check_output_path_valid(arguments.output_path, exist=True)
            if ret != CompareError.MSACCUCMP_NONE_ERROR:
    ...
    def _gen_overflow_info(res: list, overflow_type: str, detail: any) -> any:
        ...
        if status and status != 0:
            ...
            return detail.get('task_id'), detail.get('stream_id'), \
                   detail.setdefault('context_id', 65535), detail.setdefault('thread_id', 65535)
    ...
    def _is_dump_file_match(dump_file_desc: DumpFileDesc, task_info: any):
        ...
        if dump_file_desc.task_id != task_id or dump_file_desc.stream_id != stream_id:
            return False
        if context_id and thread_id \
                and dump_file_desc.context_id != context_id or dump_file_desc.thread_id != thread_id:
            return False
        return True
