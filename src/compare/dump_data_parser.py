...
class DumpDataParser:
    ...
    def _parser_overflow_info(data: any, start: int) -> dict:
        index = start
        model_id = OpDebugInfoParser.unpack_uint64_value(data, index)
        index += ConstManager.UINT64_SIZE
        stream_id = OpDebugInfoParser.unpack_uint64_value(data, index)
        index += ConstManager.UINT64_SIZE
        task_id = OpDebugInfoParser.unpack_uint64_value(data, index)
        index += ConstManager.UINT64_SIZE
        task_type = OpDebugInfoParser.unpack_uint64_value(data, index)
        index += ConstManager.UINT64_SIZE
        pc_start = OpDebugInfoParser.unpack_uint64_value(data, index)
        index += ConstManager.UINT64_SIZE
        para_base = OpDebugInfoParser.unpack_uint64_value(data, index)
    ...
    def _parser_ai_core_status(ai_core_info: dict, data: any, start: int) -> None:
        index = start
        kernel_code = OpDebugInfoParser.unpack_uint64_value(data, index)
        ai_core_info['kernel_code'] = hex(kernel_code)
        index += ConstManager.UINT64_SIZE
        block_idx = OpDebugInfoParser.unpack_uint64_value(data, index)
        ai_core_info['block_idx'] = block_idx
        index += ConstManager.UINT64_SIZE
        status = OpDebugInfoParser.unpack_uint64_value(data, index)
    ...
    def _save_op_debug_to_file(self: any, dump_path: str, output: any) -> None:
        for idx, item in enumerate(output):
            magic = OpDebugInfoParser.unpack_uint32_value(item.data, 0)
    ...
    def _parse_op_debug_old_version(self: any, dump_path: str, idx: int, item_data: any) -> map:
        ...
        dha_atomic_add_status = OpDebugInfoParser.unpack_uint64_value(item_data, index)
        dha_atomic_add_info['status'] = dha_atomic_add_status
        # parser L2 Atomic Add status
        index += ConstManager.DHA_ATOMIC_ADD_STATUS_SIZE
        l2_atomic_add_status = OpDebugInfoParser.unpack_uint64_value(item_data, index)
...
class OpDebugInfoParser:
    ...
    @staticmethod
    def unpack_uint64_value(data: any, index: int) -> int:
        return struct.unpack(ConstManager.UINT64_FMT, data[index:index + ConstManager.UINT64_SIZE])[0]

    @staticmethod
    def unpack_uint32_value(data: any, index: int) -> int:
        return struct.unpack(ConstManager.UINT32_FMT, data[index:index + ConstManager.UINT32_SIZE])[0]
    ...
    def parse_op_debug_new_version(self: any) -> map:
        ...
        for idx, key in enumerate(self.OVERFLOW_DEBUG):
            ...
            if key == 'acc_list':
                ...
                value = self.unpack_uint32_value(self.data, index)
            op_debug[key] = value
    ...
    def _parse_acc_debug_info(self: any, start: int) -> map:
        ...
        for idx, key in enumerate(self.ACC_DEBUG):
            ...
            if key == 'acc_type':
                type_value = self.unpack_uint32_value(self.data, index)
    ...
    def _parse_debug_info(self: any, start: int, acc_type: int) -> map:
        ...
        for idx, key in enumerate(data_names):
            ...
            value = self.unpack_uint64_value(self.data, index)
