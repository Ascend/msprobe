...
#    num = random.randint(0,7)
   # print(num)
    num = [0,1,2,3,3,4,5,6,7]
    for item in num:
        filename = path + "/analysis_performance_tag." + str(item) + ".xlsx"
        if os.path.exists(filename) is False:
            ...
            for i in range(int(nrows)):
                if (i-1) % int(loop) == 0 or i == 0 or (i-1) % int(loop) == 1:
            ...
            result_list={"scriptname":scriptname,"Getnext":average_liter_end_to_next_FP_start_remove_loop,"FP_BP":average_BP_and_FP_remove_loop,"bpend_iter": average_BP_end_to_iter_end_remove_loop,"wave_AR1":"Null","wave_AR2":"Null","total":average_total_time_remove_loop,"total":average_total_time_remove_loop}
            print(result_list)
            print("TrainingTime = %f" % average_total_time_remove_loop)
