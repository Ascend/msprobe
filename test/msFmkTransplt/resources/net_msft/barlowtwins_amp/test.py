...
def main():
    ...
    args.ngpus_per_node = torch.npu.device_count()
...
def main_worker(gpu, args):
    ...
    torch.distributed.init_process_group(
        backend='hccl', init_method=args.dist_url,
    ...
    model = ascend_function.TorchDistributedDataParallel(model, device_ids=[gpu])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (images, target) in enumerate(train_loader, start=epoch * len(train_loader)):
            output = model(images.npu(gpu, non_blocking=True))
            loss = criterion(output, target.npu(gpu, non_blocking=True))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if args.rank == 0:
            ...
            with torch.no_grad():
                for images, target in val_loader:
                    output = model(images.npu(gpu, non_blocking=True))
                    acc1, acc5 = accuracy(output, target.npu(gpu, non_blocking=True), topk=(1, 5))
