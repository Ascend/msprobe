...
def train(opt):
    ...
    cuda_condition = torch_npu.npu.is_available() and opt.gpu_ranks
    device = torch.device("npu:0" if cuda_condition else "cpu")

    if cuda_condition:
        model.npu()

    if cuda_condition and torch_npu.npu.device_count() > 1:
        print("Using %d GPUS for BERT" % torch_npu.npu.device_count())
