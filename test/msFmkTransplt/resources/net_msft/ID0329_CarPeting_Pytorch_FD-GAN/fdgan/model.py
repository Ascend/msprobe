...
class FDGANModel(object):
    ...
    def _init_models(self):
        ...
        e_base_model = create(self.opt.arch, cut_at_pooling=True).npu()
        e_embed_model = EltwiseSubEmbed(use_batch_norm=True, use_classifier=True, num_features=2048, num_classes=2).npu()
        self.net_E = SiameseNet(e_base_model, e_embed_model)

        di_base_model = create(self.opt.arch, cut_at_pooling=True).npu()
        di_embed_model = EltwiseSubEmbed(use_batch_norm=True, use_classifier=True, num_features=2048, num_classes=1).npu()
    ...
    def _init_losses(self):
        if self.opt.smooth_label:
            self.criterionGAN_D = GANLoss(smooth=True).npu()
            self.rand_list = [True] * 1 + [False] * 10000
        else:
            self.criterionGAN_D = GANLoss(smooth=False).npu()
            self.rand_list = [False]
        self.criterionGAN_G = GANLoss(smooth=False).npu()
    ...
    def set_input(self, input):
        ...
        self.origin = origin.npu()
        self.target = target.npu()
        self.posemap = posemap.npu()
        self.labels = labels.npu()
        self.noise = noise.npu()
