...
def extract_embeddings(model, features, alpha, query=None, topk_gallery=None, rerank_topk=0, print_freq=500):
    ...
    pairwise_score = Variable(torch.zeros(len(query), rerank_topk, 2).npu())
    ...
    for i in range(1000):
        ...
        pairwise_score[i, :, :] = model(Variable(probe_feature[i].view(1, -1).npu(), volatile=True),
                                        Variable(gallery_feature.npu(), volatile=True))
...
def extract_features(model, data_loader, print_freq=1, metric=None):
    ...
    for i, (imgs, fnames, pids, _) in enumerate(data_loader):
        ...
        outputs = extract_cnn_feature(model, imgs).npu()
        for fname, output, pid in zip(fnames, outputs, pids):
            features[fname] = output.npu()
            labels[fname] = pid.npu()
