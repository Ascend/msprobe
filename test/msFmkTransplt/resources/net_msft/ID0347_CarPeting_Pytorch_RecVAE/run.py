...
def run(model, opts, train_data, batch_size, n_epochs, beta, gamma, dropout_rate):
    ...
    for epoch in range(n_epochs):
        for batch in generate(batch_size=batch_size, device=device, data_in=train_data, shuffle=True):
            ...
            #print(prof.key_averages(group_by_icudat_shape=True).table(sort_by="self_cpu_time_total"))
            #print(prof.key_averages().table())
            prof.export_chrome_trace("simple.prof")
            if step==2:
                break
        break
...
#model = model.cuda()
model = model.to(f'npu:{device}' if isinstance(device, int) else device)
...
torch.save(model.state_dict(),"./ckpt_cuda/checkpoint.pth")
