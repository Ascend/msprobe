...
train_loader_batch_size = int(64 / int(os.getenv('NPU_WORLD_SIZE')))

# Data Loader (Input Pipeline)
train_loader = torch.utils.data.DataLoader(dataset=train_dataset,
                                           batch_size=train_loader_batch_size, # 64
                                           shuffle=False, num_workers=8, pin_memory = False, drop_last = True, sampler = train_loader_sampler)
test_loader_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset)
test_loader_batch_size = int(20 / int(os.getenv('NPU_WORLD_SIZE')))
test_loader = torch.utils.data.DataLoader(dataset=test_dataset,
                                          batch_size=test_loader_batch_size,
                                          shuffle=False, pin_memory = False, drop_last = True, sampler = test_loader_sampler)
