...
class RoIPoolTester(unittest.TestCase):
    ...
    @unittest.skipIf(not torch.npu.is_available(), "CUDA unavailable")
    def test_roi_pool_basic_cuda(self):
        ...
        assert torch.allclose(gt_y.npu(), y), 'RoIPool layer incorrect'
...
class NMSTester(unittest.TestCase):
    ...
    def test_nms_cuda(self):
        ...
        for iou in [0.2, 0.5, 0.8]:
            ...
            r_cuda = ops.nms(boxes.npu(), scores.npu(), iou)
