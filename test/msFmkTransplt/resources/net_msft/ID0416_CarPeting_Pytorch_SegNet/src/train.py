...
def train():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in train_dataloader:
            ...
            if CUDA:
                input_tensor = input_tensor.npu(GPU_ID)
                target_tensor = target_tensor.npu(GPU_ID)

            predicted_tensor, softmaxed_tensor = model(input_tensor)


            optimizer.zero_grad()
            loss = criterion(predicted_tensor, target_tensor.int())
            loss.backward()
            optimizer.step()
...
if __name__ == "__main__":
    ...
    if CUDA:
        model = SegNet(input_channels=NUM_INPUT_CHANNELS,
                       output_channels=NUM_OUTPUT_CHANNELS).npu(GPU_ID)

        class_weights = 1.0/train_dataset.get_class_probability().npu(GPU_ID)
        criterion = torch.nn.CrossEntropyLoss(weight=class_weights).npu(GPU_ID)
    else:
        model = SegNet(input_channels=NUM_INPUT_CHANNELS,
