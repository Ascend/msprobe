...
with torch.no_grad():
    ...
    image = Variable(transform(Image.open(opts.input).convert('RGB')).unsqueeze(0).npu())
    style_image = Variable(transform(Image.open(opts.style).convert('RGB')).unsqueeze(0).npu()) if opts.style != '' else None

    # Start testing
    content, _ = encode(image)

    if opts.trainer == 'MUNIT':
        style_rand = Variable(torch.randn(opts.num_style, style_dim, 1, 1).npu())
        if opts.style != '':
