...
class MUNIT_Trainer(nn.Module):
    def __init__(self, hyperparameters):
        ...
        self.s_a = torch.randn(display_size, self.style_dim, 1, 1).npu()
        self.s_b = torch.randn(display_size, self.style_dim, 1, 1).npu()
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        s_a = Variable(torch.randn(x_a.size(0), self.style_dim, 1, 1).npu())
        s_b = Variable(torch.randn(x_b.size(0), self.style_dim, 1, 1).npu())
        # encode
    ...
    def sample(self, x_a, x_b):
        ...
        s_a2 = Variable(torch.randn(x_a.size(0), self.style_dim, 1, 1).npu())
        s_b2 = Variable(torch.randn(x_b.size(0), self.style_dim, 1, 1).npu())
