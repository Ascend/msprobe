...
def _gen_excel(i):
    ...
    cmd_i = "ls " + PARSED_FILE_PATH + "/*data." + str(i) + "* | wc -l"
    ...
    if int(result_i) != 0:
        ...
        with open(combine_file, 'wb') as out_file:
            for j in range(int(result_i)):
                ...
                cmd_j = "ls " + PARSED_FILE_PATH + "/*data." + str(i) + ".slice_" + str(j)
        ...
        allreduce1_start_list = calculate_time.allreduce1_start_time()
        reduceadd1_list = calculate_time.reduceadd1_time()
        reduce1end_to_bpend_list = calculate_time.reduce1end_to_bpend_time()
        bpend_to_reduce2start_list = calculate_time.bpend_to_reduce2start_time()
        reduceadd2_list = calculate_time.reduceadd2_time()
        reduce2_to_iter_list = calculate_time.reduce2_to_iter_time()
        de.DrawExcel(output_file_name=(os.path.join(EXCEL_FILE_PATH, 'analysis_performance_tag.' + str(i) + '.xlsx')),
                     ...
                     allreduce1_start_list=allreduce1_start_list,
                     reduceadd1_list=reduceadd1_list,
                     reduce1end_to_bpend_list=reduce1end_to_bpend_list,
                     bpend_to_reduce2start_list=bpend_to_reduce2start_list,
                     reduceadd2_list=reduceadd2_list,
                     reduce2_to_iter_list=reduce2_to_iter_list)
