...
def main_worker(npu, nnpus_per_node, args):
    ...
    if args.npu is not None:
        ...
        torch.cuda.set_device('cuda:' + str(args.npu))
    ...
    criterion = nn.CrossEntropyLoss().to('cuda:' + str(args.npu))

    optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    model = model.to('cuda:' + str(args.npu))
    if args.amp:
        print("=> use amp...")
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume, map_location='cuda:' + str(args.npu))
...
def train(train_loader, model, criterion, optimizer, epoch, args, nnpus_per_node):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        #target = target.int()
        images, target = images.to('cuda:' + str(args.npu), non_blocking=True), target.to('cuda:' + str(args.npu), non_blocking=True)
