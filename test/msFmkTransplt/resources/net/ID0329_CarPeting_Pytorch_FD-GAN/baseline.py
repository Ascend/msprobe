...
def main(args):
    ...
    # Create #model
    base_model = models.create(args.arch, pretrained=False, cut_at_pooling=True, data_dir=args.data_dir).cuda()
    embed_model = EltwiseSubEmbed(use_batch_norm=True, use_classifier=True,
                                      num_features=2048, num_classes=2).cuda()
    ...
    # Optimizer
    param_groups = [
        {'params': base_model.parameters(), 'lr_mult': 1.0},
        {'params': embed_model.parameters(), 'lr_mult': 1.0}]
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.eval_step==0:
            mAP = evaluator.evaluate(val_loader, dataset.val, dataset.val, top1=False,second_stage=False)
