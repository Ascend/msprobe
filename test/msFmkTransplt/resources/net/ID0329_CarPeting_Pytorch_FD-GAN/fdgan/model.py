...
class FDGANModel(object):
    ...
    def _init_models(self):
        ...
        e_base_model = create(self.opt.arch, cut_at_pooling=True).cuda()
        e_embed_model = EltwiseSubEmbed(use_batch_norm=True, use_classifier=True, num_features=2048, num_classes=2).cuda()
        self.net_E = SiameseNet(e_base_model, e_embed_model)

        di_base_model = create(self.opt.arch, cut_at_pooling=True).cuda()
        di_embed_model = EltwiseSubEmbed(use_batch_norm=True, use_classifier=True, num_features=2048, num_classes=1).cuda()
