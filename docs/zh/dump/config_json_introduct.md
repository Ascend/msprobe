...
nan_check模式通过读取NPU寄存器状态，检测模型中的API运行时是否存在NaN或Inf的情况。该任务类型在昇腾NPU设备上运行，level须配置为"L1"。使用前请确保使用编译时带有`--include-mod=nan_check`参数的msprobe工具包。由于检测结果通过读取寄存器状态获得，因此可能出现结果正常但检测结果为溢出的情况，说明该API存在过程溢出（过程溢出为正常情况）。
