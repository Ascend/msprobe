...
- 若某些已封装 API 的部分输入或输出不需要采集，可在 [builtin_ignore_ops.yaml](../../../python/msprobe/pytorch/dump/api_dump/builtin_ignore_ops.yaml) 文件中配置内置忽略规则。该文件只影响 API forward 调用的 dump 数据内容，不影响 API 是否被封装；命中规则后，对应位置会在 dump 结果中置为`None`，常用于通信算子的接收缓冲区、原地写入类输出等不需要参与精度比对的参数。当前仅支持配置 API forward 调用的输入/输出忽略规则，不支持配置 API backward 调用中的输入/输出忽略规则。

  ```yaml
  Distributed:          # 顶层分类，使用dump结果中的API前缀
    recv:              # API名称，使用去掉顶层分类后的API名
      input: [0]       # 忽略第0个位置参数
    irecv:
      input: [0, "tensor"]  # 支持按位置参数下标或关键字参数名忽略
    isend:
      output: all      # 忽略所有输出

  NPU:
    npu_fusion_attention:
      output: [4, 5]   # 忽略第4、5个输出
  ```
  ...
  | 字段 | 说明 |
  | --- | --- |
  | 一级键 | 顶层分类，需填写 dump 结果中的 API 前缀，大小写需保持一致，例如`Functional`、`Tensor`、`Torch`、`Aten`、`VF`、`Distributed`、`NPU`、`MindSpeed`。 |
  | 二级键 | API名称，填写去掉顶层分类后的 API 名称，例如 dump 结果中为`Distributed.recv.0.forward`时填写`recv`，为`NPU.npu_fusion_attention.0.forward`时填写`npu_fusion_attention`。若 API 名称本身包含`.`，需保留`.`后的完整名称。 |
  | `input` | 需要忽略的前向输入。列表中的整数表示位置参数下标，字符串表示关键字参数名；也可配置为`all`忽略全部输入。 |
  | `output` | 需要忽略的前向输出。列表中的整数表示输出下标，字符串表示字典输出中的键；也可配置为`all`忽略全部输出。 |

  顶层分类与 [support_wrap_ops.yaml](../../../python/msprobe/pytorch/dump/api_dump/support_wrap_ops.yaml) 中注册类别的常用对应关系如下：

  | support_wrap_ops.yaml类别 | builtin_ignore_ops.yaml顶层分类 |
  | --- | --- |
  ...
  推荐先从 dump 结果或日志中的完整 API 名确认写法，再配置忽略规则。完整 API 名通常形如`<顶层分类>.<API名称>.<调用序号>.forward`，例如`Distributed.recv.0.forward`对应：
  ...
  修改该文件后需重新启动训练或推理进程，使配置重新加载。若目标是完全不采集某个 API，请修改`support_wrap_ops.yaml`；若目标是保留 API 调用记录，但屏蔽其中部分 API forward 输入或输出，请修改`builtin_ignore_ops.yaml`。
