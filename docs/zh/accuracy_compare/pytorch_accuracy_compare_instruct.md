...
| -o或--output_path                   | 配置比对结果文件存盘目录，str 类型，默认在当前目录创建output目录。默认输出csv文件，文件名称基于时间戳自动生成，格式为：`compare_result_{timestamp}.csv`。传入`--xlsx`时输出xlsx文件。<br>提示：output目录下与结果文件同名的文件将被删除覆盖。                                                      | 否       |
