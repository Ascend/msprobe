...
def _update_weights(self, weights, peft_config, base_sync_done):
    ...
    if uwc_stage3 and os.environ.get("UWC_PTR_CHECK", "0") == "1" and _uwc_ptr_before is not None:
        try:
...
async for name, weight in ensure_async_iterator(weights):
    ...
    if uwc_stage2 and diag_count < 3:
        diag_count += 1
        wf = weight.float()
        buf_float = self.buffer[offset : offset + weight.nbytes].float()
...
```python
def collect_ptr_before(model) -> dict:
