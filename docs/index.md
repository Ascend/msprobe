...
| | | verl训推权重一致性验证（UWC） | 验证veRL训推一体场景下训练侧（Megatron-core）到推理侧（vLLM）权重同步链路（bridge转换、传输、vLLM加载）的数值一致性，快速定位权重不一致来源 | [verl训推权重一致性验证（UWC）](./zh/user_guide/rl_weight_consistency_verify.md) |
